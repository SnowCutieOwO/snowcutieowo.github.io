...
> * [区域组](regions.region-flags.md#区域组)
> * [标志种类](regions.region-flags.md#标志种类)
> * [冲突标志](regions.region-flags.md#冲突标志)
> * [标志默认值](regions.region-flags.md#标志的默认值)
> * [标志列表](regions.region-flags.md#)
>   * [覆盖](regions.region-flags.md)
>   * [保护相关](regions.region-flags.md)
>   * [实体、火焰与爆炸](regions.region-flags.md)
>   * [自然事件](regions.region-flags.md)
>   * [移动](regions.region-flags.md)
>   * [地图建筑](regions.region-flags.md)
>   * [杂项](regions.region-flags.md)
...
标志拥有默认值，如果在没有对操作进行影响的区域为区域组设置标志，默认情况下就会使用这些值。摩恩行为是最敏感的。例如，如果 `item-pickup` 没有设置，WorldGuard 默认会允许这种行为。你可以通过上文已提及过的“flags”（译者注：/rg flags 区域）命令查看这个标志的默认值。状态标志的默认值也可以是允许（allow），或者不设置值。在大部分情况下，所有有关区域保护的标志，例如下文列出的那些，除 `build` 外都是没有默认值的。`build` 标志相对特殊。它的默认值取决于区域的隶属关系，通常都是自动设置的，如果没有自动设置，那么优先级最高的区域将会影响建筑操作。这就是非成员默认不能建造而成员和拥有者可以的原因。所有与保护相关的标志都与 build 标志串联检查。因此 build 标志返回的值即是该区域默认的行为。
...
|标志|种类|描述|
|---|---|---|
|passthrough|state|这个标志是“passthrough build”的缩写，它和移动没有任何关系。<ul><li>如果没有设置**（默认）**，那么区域会保护其中的方块；</li><li>如果设置为 `deny`，那么区域会保护其中的方块；</li><li>如果设置为 `allow`，那么区域不再会保护其中的方块。</ul>这个标志是干什么用的？<ul><li>在你使用其他标志（例如 PVP、治疗标志等）且你不想要阻止玩家的建筑行为；</li><li>为什么不把 `build` 标志设置为 `allow`（稍后解释）呢？这会覆盖其他区域的设置并允许玩家进行建筑！</li></ul>
...
|标志|种类|描述|
|---|---|---|
|build|state|对于全部玩家或方块：<ul><li>方块是否可以被放置或破坏；</li><li>门、拉杆等（但不限制在容器中的）是否可以使用；</li><li>实体和方块是否可以交互；</li><li>是否允许 PVP；</li><li>是否允许在床上睡觉；</li><li>是否能打开容器；</li><li>是否能放置载具（船、矿车等）；</li><li>等等...</li></ul>|
|interact|state|包括所有能够“交互”的方块或实体：<ul><li>门、拉杆等（但不限制在容器中的）是否可以使用；</li><li>载具（包括动物）是否可以骑乘；</li><li>等等...</li>|
